			...
			HttpService.get(url).then(function(data){
				...
				$scope.isError = false;
				 if (data.data && data.data.length != 0) {
					 ...
					 renderPieChart(divId, data, 'Most Used Models',36);
				 } else {
					 $scope.isNoDataDB = true;
					 $scope.msg = $scope.msg2;
				 }
			...
			HttpService.get(url).then(function(data){
				...
				$scope.isError = false;
		    	 if (data && data.length != 0) {
					 ...
					 renderPieChart(divId, data, 'Most Used Wash Cycles',44);
		    	 } else {
		    		 $scope.isNoDataDB = true;
		    		 $scope.msg = $scope.msg2;
		    	 }
				...
				$scope.isError = false
		    	 ...
		    	 if (data && data.length != 0) {
		             ...
		             if (total > 0)	{
						 ...
						 renderPieChart(divId, data, 'Not Connected Machines',45);
		    	 	} else {
	...
	$scope.plotChartFunction = function(divId){
		...
		var param = $scope.usagedata;
      	HttpService.post(url, param).then(function(data){
			...
			$scope.progress = false;
			$scope.timeScales=timeScales;
			$scope.maxLineChartData=lineChartSeriesData;
